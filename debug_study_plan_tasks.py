...
def test_study_plan_tasks_consistency():
    print("=== 测试学习计划任务数一致性 ===")
    ...
    if response.status_code != 200:
        print(f"获取学习计划列表失败: {response.status_code}")
        return
    ...
    # 2. 检查每个学科的详细任务
    print(f"\n2. 检查每个学科的详细任务...")
    for subject in subjects[:3]:  # 只检查前3个学科
        ...
        if subject_name:
            ...
            if response.status_code == 200:
                ...
                print(f"  获取{subject_name}详情失败: {response.status_code}")

if __name__ == "__main__":
    test_study_plan_tasks_consistency()